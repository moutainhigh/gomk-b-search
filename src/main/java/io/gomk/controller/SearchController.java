...
public class SearchController extends SuperController {
	...
	@GetMapping("/zb")
	public ResponseData<PageResult<Page<List<SearchResultVO>>>> searchZB(int page, int pageSize, String keyWord)
			throws IOException {
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "keyWord", value = "关键字", required = false, paramType = "query", dataType = "String", defaultValue = "设备") })
	@GetMapping("/zgyq")
	public ResponseData<PageResult<Page<List<SearchResultVO>>>> searchZGYQ(int page, int pageSize, String keyWord)
			throws IOException {
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "keyWord", value = "关键字", required = false, paramType = "query", dataType = "String", defaultValue = "设备") })
	@GetMapping("/zjcg")
	public ResponseData<PageResult<Page<List<SearchResultVO>>>> searchZJCG(int page, int pageSize, String keyWord)
			throws IOException {
		// return execSearch(zjcgIndex, page, pageSize, keyWord);
		return execSearch(zjcgIndex, page, pageSize, keyWord, false);
	}

	private ResponseData<PageResult<Page<List<SearchResultVO>>>> execSearch(String indexName, int page, int pageSize,
			String keyWord, Boolean bl) throws IOException {
		...
		// sourceBuilder.query(QueryBuilders.matchQuery("content",
		 ...
		 * .field("age")); sourceBuilder.aggregation(aggregation);
		 */

		// 做查询建议
		/*
		 * SuggestionBuilder termSuggestionBuilder =
		 * SuggestBuilders.termSuggestion("user").text("kmichy"); SuggestBuilder
		...
		for (SearchHit hit : searchHits) {
			...
			// 从map中取字段值
			/*
			 * String documentTitle = (String) sourceAsMap.get("title"); List<Object> users
			 * = (List<Object>) sourceAsMap.get("user"); Map<String, Object> innerObject =
...
//            }
		...
		// 获取聚合结果
		/*
		 * Aggregations aggregations = searchResponse.getAggregations(); Terms
		 * byCompanyAggregation = aggregations.get("by_company"); Bucket elasticBucket =
		 * byCompanyAggregation.getBucketByKey("Elastic"); Avg averageAge =
		 * elasticBucket.getAggregations().get("average_age"); double avg =
		 * averageAge.getValue();
		 */

		// 获取建议结果
		/*
		 * Suggest suggest = searchResponse.getSuggest(); TermSuggestion termSuggestion
		 * = suggest.getSuggestion("suggest_user"); for (TermSuggestion.Entry entry :
		 * termSuggestion.getEntries()) { for (TermSuggestion.Entry.Option option :
		 * entry) { String suggestText = option.getText().string(); } }
		 */

		// 异步方式发送获查询请求
		/*
		 * ActionListener<SearchResponse> listener = new
		 * ActionListener<SearchResponse>() {
		 * 
		 * @Override public void onResponse(SearchResponse getResponse) { //结果获取 }
		 * 
		 * @Override public void onFailure(Exception e) { //失败处理 } };
